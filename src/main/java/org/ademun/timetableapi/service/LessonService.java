...
public class LessonService {
  ...
  public List<Lesson> all() {
    ...
    return lessonRepository.findAll().stream().sorted(Comparator.comparing(Lesson::getId)).toList();
  }

  @Transactional
  public List<Lesson> byWeek(int week) {
    log.info("Retrieving Lessons by week {}", week);
    return lessonRepository.findAll().stream().filter(lesson -> lesson.getDay().getWeek() == week)
        .toList();
  }
  ...
  public List<Lesson> byWeekDay(int week, int day) {
    ...
    return byWeek(week).stream()
        .filter(lesson -> (lesson.getDay().getId() - (7L * (week - 1)) == day)).toList();
  }
  ...
  public List<Lesson> byWeekDayOrder(int week, int day, int order) {
    ...
    return byWeekDay(week, day).stream()
        .filter(lesson -> Objects.equals(lesson.getOrder().getId(), order)).toList();
  }
