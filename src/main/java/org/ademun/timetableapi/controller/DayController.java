...
public class DayController {
  ...
  public CollectionModel<EntityModel<Day>> all() {
    List<EntityModel<Day>> dayModelList =
        dayService.all().stream().map(dayMapper::toModel).toList();
    return CollectionModel.of(dayModelList,
        linkTo(methodOn(DayController.class).all()).withSelfRel());
  }
