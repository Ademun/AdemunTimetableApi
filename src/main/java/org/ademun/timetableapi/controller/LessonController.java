...
public class LessonController {
  ...
  public CollectionModel<EntityModel<Lesson>> all() {
    List<EntityModel<Lesson>> lessonModelList =
        lessonService.all().stream().map(lessonAssembler::toModel).toList();
    return CollectionModel.of(lessonModelList,
  ...
  public CollectionModel<EntityModel<Lesson>> byWeek(@PathVariable int week) {
    List<EntityModel<Lesson>> lessonModelList =
        lessonService.byWeek(week).stream().map(lessonAssembler::toModel).toList();
    return CollectionModel.of(lessonModelList,
        linkTo(methodOn(LessonController.class).all()).withSelfRel());
  }

  @GetMapping(value = "/{week}/{day}")
  public CollectionModel<EntityModel<Lesson>> byWeekDay(@PathVariable int week,
      @PathVariable int day) {
    List<EntityModel<Lesson>> lessonModelList =
        lessonService.byWeekDay(week, day).stream().map(lessonAssembler::toModel).toList();
    return CollectionModel.of(lessonModelList,
        linkTo(methodOn(LessonController.class).all()).withSelfRel());
  }

  @GetMapping(value = "/{week}/{day}/{order}")
  public CollectionModel<EntityModel<Lesson>> byWeekDayOrder(@PathVariable int week,
      @PathVariable int day, @PathVariable int order) {
    List<EntityModel<Lesson>> lessonModelList =
        lessonService.byWeekDayOrder(week, day, order).stream().map(lessonAssembler::toModel)
            .toList();
    return CollectionModel.of(lessonModelList,
        linkTo(methodOn(LessonController.class).all()).withSelfRel());
  }
